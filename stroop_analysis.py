...
sham_color = '#C2E6E1'
vns_color = '#DBD8E9'
sham_edge_color = '#7CC7C7'
vns_edge_color = '#A794C3'

# 바 그래프 그리기
fig, ax = plt.subplots()
width = 0.35
x = np.array(range(len(sham_means)))
bars1 = ax.bar(x - width/2, sham_means, width, yerr=sham_sems, label='Sham', capsize=5, color=sham_color, edgecolor=sham_edge_color)
bars2 = ax.bar(x + width/2, vns_means, width, yerr=vns_sems, label='VNS', capsize=5, color=vns_color, edgecolor=vns_edge_color)

# 그래프 설정
ax.set_xlabel('Session')
ax.set_ylabel('Stroop Task Integrated Score')
ax.set_title('Reaction time, [0, 1], [0.33, 0.33, 0.33]')
ax.set_xticks(x)
...
ax.set_xlabel('Session')
ax.set_ylabel('Stroop Task Integrated Score')
ax.set_title('Accuracy, [0, 1], [0.33, 0.33, 0.33]')
ax.set_xticks(x)
ax.set_xticklabels(x)
...
#     # vns_data, sham_data = [], []
#     for se in sessions:
#         vns = msdict_save[n][str(se)]['VNS'] # - msdict_save[n][str([0,1])]['VNS']
#         sham = msdict_save[n][str(se)]['sham'] # - msdict_save[n][str([0,1])]['sham']
...
#         t_stat, p_value = stats.ttest_ind(msdict_save[n][str(se)]['sham'], msdict_save[n][str(se)]['VNS'])
#         print()
#         print(n, se)
#         print("t-statistic:", t_stat)
#         print("p-value:", p_value)
...
#     if True:
#         fsave = r'C:\\SynologyDrive\worik in progress\혁창패\임상실험 논문작성\result-figure'
#         fsave2 = fsave + '\\Figure1C_' + str(ix) + '.png'
#         plt.tight_layout()
#         plt.savefig(fsave2, dpi=200 ,bbox_inches='tight', pad_inches=0.1)
#         plt.show()
...
#         session_labels.extend([f'Session {i+1}'] * len(msdict_save[n][str(se)]['sham']))
        
#         all_data.extend(msdict_save[n][str(se)]['VNS'])
#         group_labels.extend(['VNS'] * len(msdict_save[n][str(se)]['VNS']))
#         session_labels.extend([f'Session {i+1}'] * len(msdict_save[n][str(se)]['VNS']))
...
#     sham_violin_line_color = '#ABD8CD'
#     vns_violin_line_color = '#BD89D6'

#     sns.violinplot(x=session_labels, y=all_data, hue=group_labels, split=True, inner=None, ax=ax,
#                palette={'Sham': sham_color, 'VNS': vns_color}, alpha=0.5)

#     # Scatter plot for individual data points with specified colors
#     for i, se in enumerate(sessions):
#         x_vals_vns = np.full(len(msdict_save[n][str(se)]['VNS']), i + 0.2)
#         x_vals_sham = np.full(len(msdict_save[n][str(se)]['sham']), i - 0.2)
    
#         ax.scatter(x_vals_sham, msdict_save[n][str(se)]['sham'], color=sham_scatter_color, label='Sham' if i == 0 else "", alpha=0.4)
#         ax.scatter(x_vals_vns, msdict_save[n][str(se)]['VNS'], color=vns_scatter_color, label='VNS' if i == 0 else "", alpha=0.4)

#     for i, se in enumerate(sessions):
#         # Sham
#         sham_data = msdict_save[n][str(se)]['sham']
#         q1, median, q3 = np.percentile(sham_data, [25, 50, 75])
#         ax.add_patch(plt.Rectangle((i - 0.1, q1), 0.05, q3 - q1, edgecolor='#A5DAD2', facecolor='#A5DAD2', alpha=0.7))
#         ax.plot([i - 0.1, i - 0.05], [median, median], color='#8CC4B9', alpha=1.0)
        
#         # VNS
#         vns_data = msdict_save[n][str(se)]['VNS']
#         q1, median, q3 = np.percentile(vns_data, [25, 50, 75])
#         ax.add_patch(plt.Rectangle((i + 0.05, q1), 0.05, q3 - q1, edgecolor='#BDB9D6', facecolor='#C9C7E3', alpha=0.7))
#         ax.plot([i + 0.05, i + 0.1], [median, median], color='#BDB9D6', alpha=1.0)
...
#     x_vals = np.arange(len(sessions))  # 동일한 x 위치 사용
#     # Scatter plot for individual data points
#     for i, se in enumerate(sessions):
#         x_vals_vns = np.full(len(msdict_save[n][str(se)]['VNS']), i + 0.2)
#         x_vals_sham = np.full(len(msdict_save[n][str(se)]['sham']), i - 0.2)
        
#         ax.scatter(x_vals_sham, msdict_save[n][str(se)]['sham'], color=sham_scatter_color, label='Sham' if i == 0 else "", alpha=0.3)
#         ax.scatter(x_vals_vns, msdict_save[n][str(se)]['VNS'], color=vns_scatter_color, label='VNS' if i == 0 else "", alpha=0.3)
...
for ix, n in enumerate(dict_name):
    ...
    sns.violinplot(x=session_labels, y=all_data, hue=group_labels, split=True, inner=None, ax=ax,
                   palette={'Sham': sham_color, 'VNS': vns_color}, alpha=1)
    ...
    # Adding quartile bars (box plot)
    for i, se in enumerate(sessions):
        # Sham
        sham_data = msdict_save[n][str(se)]['sham']
        q1, median, q3 = np.percentile(sham_data, [25, 50, 75])
        ax.add_patch(plt.Rectangle((i - 0.1, q1), 0.05, q3 - q1, edgecolor='#A5DAD2', facecolor='#A5DAD2', alpha=0.9))
        ax.plot([i - 0.1, i - 0.05], [median, median], color='#8CC4B9', alpha=1.0)
        
        # VNS
        vns_data = msdict_save[n][str(se)]['VNS']
        q1, median, q3 = np.percentile(vns_data, [25, 50, 75])
        ax.add_patch(plt.Rectangle((i + 0.05, q1), 0.05, q3 - q1, edgecolor='#BDB9D6', facecolor='#C9C7E3', alpha=0.9))
        ax.plot([i + 0.05, i + 0.1], [median, median], color='#BDB9D6', alpha=1.0)
